  ...
  return (
    ...
    >
      ...
      <Card className="grid gap-8 p-5 md:grid-cols-[320px_1fr] md:p-8">
        <ImageWithFallback src={shop.imageUrl} alt={`${shop.name}の画像`} className="h-72 w-full rounded-2xl object-cover" />
        <div>
          <p className="text-sm font-black text-blue-700">#{shop.number}</p>
          <h1 className="mt-2 text-4xl font-black text-slate-900">{shop.name}</h1>
          <p className="mt-2 text-lg font-semibold text-slate-600">{shop.organization}</p>
          ...
          <div className="mt-6 flex flex-wrap gap-3">
            <ButtonLink href="/map">マップに戻る</ButtonLink>
