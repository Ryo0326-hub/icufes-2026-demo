    ...
    >
      <div className="mx-auto max-w-5xl">
        <Card className="grid gap-6 p-4 sm:p-5 md:grid-cols-[280px_1fr] md:gap-8 md:p-8 lg:grid-cols-[320px_1fr]">
          <ImageWithFallback src={event.imageUrl} alt={`${event.title}の画像`} className="h-56 w-full rounded-2xl object-cover sm:h-72" />
          <div>
            <Badge>{categoryLabels[event.category]}</Badge>
            <h1 className="mt-3 text-3xl font-black leading-tight text-slate-900 sm:text-4xl">{event.title}</h1>
            <p className="mt-2 text-base font-semibold text-slate-600 sm:text-lg">{event.organization}</p>
